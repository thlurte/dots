...
Singleton {
  ...
  // Incognito mode - temporarily pause clipboard watching
  ...
  onActiveChanged: {
    if (root.active && !root.incognitoMode) {
      startWatchers();
    } else {
      stopWatchers();
      if (!root.active) {
  ...
  // Start/stop watchers when incognito mode changes
  ...
  function startWatchers() {
    if (!root.active || !autoWatch || watchersStarted || !root.cliphistAvailable || root.incognitoMode)
      return;
  ...
  function deleteById(id) {
    console.log("[ClipboardService] deleteById called with id:", id);
    if (!root.cliphistAvailable) {
      console.log("[ClipboardService] cliphist not available, aborting delete");
      return;
    }
    console.log("[ClipboardService] Executing cliphist delete for id:", id);
